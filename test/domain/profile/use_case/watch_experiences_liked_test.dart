...
void main() {
  ...
  WatchExperiencesLiked useCase;
  setUpAll(
    () {
      configureDependencies(injectable.Environment.test);
      mockProfileRepository = getIt<ProfileRepositoryInterface>();
      useCase = WatchExperiencesLiked(mockProfileRepository);
    },
  );
  ...
  test(
    ...
    () async {
      // Arrange
      when(mockProfileRepository.watchExperiencesLiked(any)).thenAnswer((_) => createStream(right(experiencesLiked)));
      // Act
    ...
    () {
      test(
        ...
        () async {
          ...
          when(mockProfileRepository.watchExperiencesLiked(any)).thenAnswer((_) => createStream(left(failure)));
          // Act
          final result = await _act(useCase, params);
          // Assert
...
Future<Either<Failure, KtList<Experience>>> _act(WatchExperiencesLiked useCase, Params params) async {
...
void _verifyInteractions(ProfileRepositoryInterface mockProfileRepository) {
  verify(mockProfileRepository.watchExperiencesLiked(any));
