...
void main() {
  ...
  final usersFoundByName = KtSet.of(User.empty());
  test(
    "Should return a Set of Users",
    () async {
      // Arrange
      when(mockSearchRepository.searchUsersByName(any)).thenAnswer((_) => createStream(right(usersFoundByName)));
      // Act
    ...
    () {
      test(
        ...
        () async {
          ...
          when(mockSearchRepository.searchUsersByName(any)).thenAnswer((_) => createStream(left(failure)));
          // Act
          final result = await _act(useCase, params);
          // Assert
...
Future<Either<Failure, KtSet<User>>> _act(SearchUsersByName useCase, Params params) async {
