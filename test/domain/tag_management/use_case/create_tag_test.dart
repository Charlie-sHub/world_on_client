  ...
  group(
    ...
    () {
      test(
        ...
        () async {
          // Arrange
          when(mockTagRepository.createTag(any)).thenAnswer((_) async => left(const CoreFailure.serverError()));
          // Act
          final result = await useCase(Params(tag: tag));
          // Assert
      ...
      test(
        ...
        () async {
          // Arrange
          when(mockTagRepository.createTag(any)).thenAnswer((_) async => left(const CoreFailure.nameAlreadyInUse()));
          // Act
          final result = await useCase(Params(tag: tag));
          // Assert
